...
# Функция для очистки сегодняшних задач
def clear_today_tasks():
    today = datetime.now().strftime('%Y-%m-%d')
    cursor.execute("DELETE FROM tasks WHERE date=?", (today,))
    conn.commit()
    logging.info("Сегодняшние задачи были удалены из расписания.")
...
def show_main_menu(message):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    buttons = ["Получить погоду", "Запланировать задачу", "Посмотреть расписание", "Посмотреть вчерашнее расписание", "Помощь"]
    keyboard.add(*buttons)
...
# Обработчик для просмотра вчерашнего расписания
@bot.message_handler(func=lambda message: message.text == "Посмотреть вчерашнее расписание")
def show_yesterdays_schedule(message):
    ...
    if not tasks:
        bot.send_message(message.chat.id, "У вас не было запланированных задач на вчера.")
    else:
        schedule_message = "\n".join([f"{task[2]} в {task[3]}" for task in tasks])
        bot.send_message(message.chat.id, f"Ваше вчерашнее расписание:\n{schedule_message}")
...
def handle_task_response(message):
    ...
    # Возврат к главному меню после ответа
    show_main_menu(message)
